...
const L: Record<LangCode, {
  ...
  bg: {
    ...
    hint: "Подсказване", question: (c, t) => `Въпрос ${c} от ${t}`,
...
const CONTEXT_D: Record<LangCode, Record<TopicContext, {
  ...
  bg: {
    weak: {
      ...
      practiceSupport: "Не се притеснявай — тази тема е трудна. Нека помислим заедно!",
      hintPrefix: "💡 Подсказване:",
    ...
    if (problem.includes("Преобразувай") && problem.includes("дроб")) {
      ...
      if (decimalMatch) {
        ...
        if (decimal === "0.5") {
          ...
          return "0.75 означава седемдесет и пет стотни, което е дробта 75/100. Можем да я съкратим до 3/4.";
        } else if (decimal === "0.2") {
          return "0.2 означава две десети, което е дробта 2/10. Можем да я съкратим до 1/5.";
        } else if (decimal === "0.1") {
          return "0.1 означава една десета, което е дробта 1/10.";
        }
      }
      return "Когато преобразуваш десетична дроб в обикновена, цифрите след запетаята стават числител, а знаменателят зависи от броя цифри.";
    }
    return undefined;
  };
